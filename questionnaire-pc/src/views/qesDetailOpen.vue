          ...
          <div class="quesItem">
            ...
            >
              <table>
                ...
                <tr
                  v-for="(q,w) in a.auto_grow.answer"
                  :key="w"
                >
                  <td
                    v-for="(b,j) in a.auto_grow.answer[w]"
                    :key="j"
                  >
                    <input
                      type="text"
...
import { HTTP_AccurateOpen, HTTP_AccurateOpenSave } from "@/http/interface";
import { datefmt } from "@/utils/common.js";
...
export default {
  ...
  methods: {
    /* 获取题型 */
    getData() {
      HTTP_AccurateOpen(this.$route.query.qid).then((res) => {
        ...
        list.forEach((item, index) => {
          ...
          if (item.type == 13) {
            ...
            item.auto_grow.answer.push(arr);
          }
          if (item.cate > 1) {
            if (item.wu) {
    ...
    zzbg(e) {
      ...
      this.list[i].auto_grow.answer[row][col] = e.data;
    },
    /* 添加行 */
    addRow(a) {
      let arr = [];
    ...
    save() {
      ...
      let time = datefmt();
      HTTP_AccurateOpenSave(time, this.$route.query.qid, write).then((res) => {
        this.$dialog.alert({
          title: "提交成功",
          message: "问卷已经提交成功！",
        }).then(() => {
