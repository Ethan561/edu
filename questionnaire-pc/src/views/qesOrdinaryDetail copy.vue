    ...
    >
      <van-swipe-item v-for="(b, j) in list" :key="j" class="qesList">
        ...
        <div class="flex-between">
          <van-button color="#176095" type="info">上一部分</van-button>
          <van-button color="#176095" @click="checkAnswer(b,j)" type="info">下一部分</van-button>
        </div>
      </van-swipe-item>
    </van-swipe>
        ...
        for (let i = 0; i < list.length; i++) {
          ...
          if (a.type == 1 && a.must == 1) {
            if (!a.single_choice.answer) {
              this.$toast(
                `第${arrNum[index]}部分,第${i + 1}题为必填类型，请完成答题`
              );
              return false;
            }
          }
